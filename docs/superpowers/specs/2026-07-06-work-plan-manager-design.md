...
| 主题 | 支持浅色/深色主题,工具栏一键切换,选择持久化,重启后保持;默认浅色 |
...
│   │   └── theme.py            浅色/深色主题:QPalette + QSS 定义与切换
│   ├── services/
│   │   ├── plan_service.py
│   │   ├── category_service.py
│   │   └── reminder_service.py
...
CREATE TABLE settings (
    key         TEXT PRIMARY KEY,           -- 如 'theme'
    value       TEXT NOT NULL               -- 'light' / 'dark'
);
```
...
- 工具栏最右侧主题切换按钮(月亮/太阳图标):浅色 ↔ 深色一键切换,立即全局生效,写入 `settings` 表持久化

### 主题(浅色/深色)

- `theme.py` 定义两套 QPalette + QSS;所有控件颜色一律取自主题,不在业务代码里写死色值
- 自绘月历控件(格线、日期数字、今天高亮、逾期标记)从当前主题调色板取色;分类颜色由用户自定,两种主题下都直接使用,计划条上的文字自动按底色亮度选黑/白保证可读
- 启动时从 `settings` 表读取上次主题,默认浅色
...
- **pytest 覆盖服务层与数据层**(使用临时 SQLite 文件,无 GUI 依赖):
  ...
  - 备份滚动保留 5 份的逻辑
  - 设置读写:主题偏好保存与读取,缺省值为浅色
  - 计划条文字黑/白自动选择:按分类底色亮度计算
- **GUI 手动冒烟清单**(打包后在 Windows 上过一遍):
  ...
  8. 切换深色主题 → 全部界面(含月历、对话框)变色无遗漏;重启后仍是深色
